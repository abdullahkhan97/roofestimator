@extends('layouts.app', ['title' => 'New Roof Estimate', 'pageTitle' => 'New estimate'])

@php
    $value = fn ($name, $default = 0) => old($name, $default);
    $field = function ($name, $label, $unit = null, $default = 0) use ($value) {
        $html = '<div class="span-3 field"><label for="'.$name.'">'.$label.($unit ? ' <span class="muted">('.$unit.')</span>' : '').'</label>';
        $html .= '<input id="'.$name.'" name="'.$name.'" type="number" min="0" step="0.01" value="'.e($value($name, $default)).'">';
        $html .= '</div>';
        return $html;
    };
@endphp
...
@section('content')
    <form class="grid" method="POST" action="{{ route('estimates.store') }}">
        @csrf

        <section class="span-12" style="margin-bottom: 4px;">
            <h1>Master residential steep estimate</h1>
            <p class="muted">This input screen follows the workbook Inputs tab. Roof system, pitch, and complexity drive line-item pricing from the Rates tab.</p>
        </section>
        ...
        <section class="span-8 panel">
            ...
            <div class="grid">
                <div class="span-6 field">
                    ...
                    <input id="job_name" name="job_name" value="{{ old('job_name') }}">
                </div>
                <div class="span-6 field">
                ...
                <div class="span-12 field">
                    ...
                    <input id="project_address" name="project_address" value="{{ old('project_address') }}">
                </div>
            </div>
        </section>
            ...
            <div class="field">
                ...
                <select id="roof_type_id" name="roof_type_id" required>
                    @foreach($roofTypes as $roofType)
                        <option value="{{ $roofType->id }}" @selected(old('roof_type_id') == $roofType->id)>{{ $roofType->name }} - {{ number_format($roofType->margin_percent, 2) }}% margin</option>
                    @endforeach
                </select>
            </div>
            <div class="field">
                ...
                <select id="roof_pitch_id" name="roof_pitch_id" required>
                    @foreach($pitches as $pitch)
                        <option value="{{ $pitch->id }}" @selected(old('roof_pitch_id') == $pitch->id)>{{ $pitch->label }} ({{ number_format($pitch->multiplier, 2) }}x)</option>
                    @endforeach
                </select>
            </div>
            <div class="field">
                ...
                <select id="roof_complexity_id" name="roof_complexity_id" required>
                    @foreach($complexities as $complexity)
                        <option value="{{ $complexity->id }}" @selected(old('roof_complexity_id') == $complexity->id)>{{ $complexity->label }} ({{ number_format($complexity->multiplier, 2) }}x)</option>
                    @endforeach
                </select>
            </div>
        ...
        <section class="span-12 actions">
            <button class="button blue" type="submit">Calculate estimate</button>
        </section>
    </form>
@endsection
